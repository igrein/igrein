...
async def generate_answer(question: str, profile_data: dict, is_medical: bool = False) -> str:
    ...
    if is_medical:
        prompt = f"""
Ты ветеринар-ратолог, специалист по болезням ежей. Пользователь обратился с вопросом о состоянии здоровья своего питомца. Отвечай на вопросы ТОЛЬКО о ежах.
ЕСЛИ ВОПРОС НЕ О ЕЖАХ - ответь: "Я специализируюсь только на вопросах содержания ежей и не могу ответить на этот вопрос."
...
"""
        ...
        prompt = f"""
Ты консультант по содержанию ежей. Отвечай на вопросы ТОЛЬКО о ежах, развернуто, но простым языком..
ЕСЛИ ВОПРОС НЕ О ЕЖАХ - ответь: "Я специализируюсь только на вопросах содержания ежей и не могу ответить на этот вопрос."
